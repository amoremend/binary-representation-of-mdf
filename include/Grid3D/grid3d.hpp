...
public:
	...
	float getCellCostMan(const float &_x, const float &_y, const float &_z){
...
#pragma GCC diagnostic pop
	...
	void computeManhattanGrid(int mask_bits = 64);
	...
	void buildGridSliceMsg(float z)
	{
		...
		float maxProb = -1.0;
		for(int i=offset; i<end; i++)
			if(m_grid[i].man > maxProb)
				maxProb = m_grid[i].man;
		...
		maxProb = 100.0/maxProb;
		for(int i=0; i<m_gridSizeX*m_gridSizeY; i++)
			m_gridSliceMsg.data[i] = (int8_t)(m_grid[i+offset].man*maxProb);
	}
