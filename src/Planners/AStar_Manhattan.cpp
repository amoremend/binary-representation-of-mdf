...
AStar_Manhattan::AStar_Manhattan(bool _use_3d = true, std::string _name = "astar" ): AlgorithmBase(_use_3d, _name){
    configAlgorithm();
}
    
AStar_Manhattan::AStar_Manhattan(bool _use_3d = true): AlgorithmBase(_use_3d, "astar")
{
    configAlgorithm();
}
void AStar_Manhattan::configAlgorithm(){
...
void AStar_Manhattan::publishOccupationMarkersMap()
{
#ifdef ROS
...
void AStar_Manhattan::publishROSDebugData(const Node* _node, const T &_open_set, const U &_closed_set)
{
...
inline Vec3i AStar_Manhattan::getVectorPull(const Vec3i &_source, const Vec3i &_target){
    return {_target.x - _source.x, _target.y - _source.y, _target.z - _source.z};
}

// JAC: Create a file --> computeGradient
inline float AStar_Manhattan::computeGradient(const Node* _current, Node* _suc,  unsigned int _n_i, unsigned int _dirs){
    unsigned int cost;
...
inline unsigned int AStar_Manhattan::computeG(const Node* _current, Node* _suc,  unsigned int _n_i, unsigned int _dirs){
...
void AStar_Manhattan::exploreNeighbours(Node* _current, const Vec3i &_target, node_by_position &_index_by_pos){
...
PathData AStar_Manhattan::findPath(const Vec3i &_source, const Vec3i &_target)
{
