#include "Planners/LazyThetaStarM2_Manhattan.hpp"

namespace Planners
{
    LazyThetaStarM2_Manhattan::LazyThetaStarM2_Manhattan(bool _use_3d):ThetaStarM2(_use_3d, "lazythetastarm2") {}
    LazyThetaStarM2_Manhattan::LazyThetaStarM2_Manhattan(bool _use_3d, std::string _name = "lazythetastarm2" ):ThetaStarM2(_use_3d, _name) {}
    
    void LazyThetaStarM2_Manhattan::SetVertex(Node *_s_aux)
    {   
        if (!LineOfSight::bresenham3DWithMaxThreshold(_s_aux->parent, _s_aux, discrete_world_, max_line_of_sight_cells_)){
            ...
            for (const auto &i: direction)
            {
                ...
                if ( successor2->isInClosedList ) 
                {
                    ...
                    G_new = successor2->G +  geometry::distanceBetween2Nodes_Man(successor2, _s_aux) + cost_term;  
                    if (G_new < G_max)
                    {
                        G_max = G_new;
    ...
    inline void LazyThetaStarM2_Manhattan::ComputeCost(Node *_s_aux, Node *_s2_aux)
    {
        /* *********************************************************************************************************** */
        // Check the execution
        // std::cout << "s_aux X: " << _s_aux->coordinates.x << std::endl;
        // std::cout << "s_aux Y: " << _s_aux->coordinates.y << std::endl;
        // std::cout << "s_aux Z: " << _s_aux->coordinates.z << std::endl;
        // std::cout << "s_aux COST: " << _s_aux->cost << std::endl;

        // std::cout << "s_aux->parent X: " << _s_aux->parent->coordinates.x << std::endl;
        // std::cout << "s_aux->parent Y: " << _s_aux->parent->coordinates.y << std::endl;
        // std::cout << "s_aux->parent Z: " << _s_aux->parent->coordinates.z << std::endl;
        // std::cout << "s_aux->parent COST: " << _s_aux->parent->cost << std::endl;

        // std::cout << "s2_aux X: " << _s2_aux->coordinates.x << std::endl;
        // std::cout << "s2_aux Y: " << _s2_aux->coordinates.y << std::endl;
        // std::cout << "s2_aux Z: " << _s2_aux->coordinates.z << std::endl;
        // std::cout << "s2_aux COST: " << _s2_aux->cost << std::endl;
        /* *********************************************************************************************************** */

        // Like Original Lazy Theta Star
        auto dist2   = geometry::distanceBetween2Nodes_Man(_s_aux->parent, _s2_aux);
        ...
        // as a neighbour node
        LineOfSight::bresenham3D_Man_WON(_s_aux->parent, _s2_aux, checked_nodes);
    ...
    inline unsigned int LazyThetaStarM2_Manhattan::computeG(const Node* _current, Node* _suc,  unsigned int _n_i, unsigned int _dirs){
    ...
    PathData LazyThetaStarM2_Manhattan::findPath(const Vec3i &_source, const Vec3i &_target)
    {
