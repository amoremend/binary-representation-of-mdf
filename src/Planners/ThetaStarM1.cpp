...
namespace Planners
{
    ...
    inline void ThetaStarM1::ComputeCost(Node *_s_aux, Node *_s2_aux)
    {
        ...
        if (LineOfSight::bresenham3D((_s_aux->parent), _s2_aux, discrete_world_, checked_nodes))
        {
            ...
            auto cost_term = static_cast<unsigned int>(cost_weight_ * (1/(((static_cast<double>(_s_aux->parent->cost) + static_cast<double>(_s2_aux->cost))/2) * dist_scale_factor_reduced_))) * n_checked_nodes; 
            ...
            auto cost_term = static_cast<unsigned int>(cost_weight_ * (1/(((static_cast<double>(_s_aux->cost) + static_cast<double>(_s2_aux->cost))/2) * dist_scale_factor_reduced_)));
