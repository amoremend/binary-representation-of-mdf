...
namespace Planners
{
    namespace utils
    {
        namespace geometry
        {
            ...
            // Manhattan distance
            unsigned int distanceBetween2Nodes_Man(const Node &_n1, const Node &_n2)
            {
                return distanceBetween2Nodes_Man(_n1.coordinates, _n2.coordinates);
            }
            unsigned int distanceBetween2Nodes_Man(const Node *_n1, const Node *_n2)
            {
                return distanceBetween2Nodes_Man(*_n1, *_n2);
            }
            unsigned int distanceBetween2Nodes_Man(const Vec3i &_v1, const Vec3i &_v2)
            {
                int dx = std::abs(_v1.x - _v2.x);
                int dy = std::abs(_v1.y - _v2.y);
                int dz = std::abs(_v1.z - _v2.z);

                return static_cast<unsigned int>(dist_scale_factor_*(dx + dy + dz));
            }
